...
def __CVectorsForGaussBasis(refImage, kernelShape, gaussList, \
    ...
    for aGauss in gaussList:
        ...
        gaussK = __gauss(shape=(kh,kw), center=center, \
                         sx=aGauss['sx'], sy=aGauss['sy'])

        if badPixMask is not None:
            newC = [np.ma.array(signal.convolve2d(refImage, gaussK * aU * aV, \
                                             mode='same'), mask = badPixMask) \
...
def __coeffsToKernel(coeffs, gaussList, kernelShape = (10,10)):
    ...
    if gaussList is None:
        ...
        for aGauss in gaussList:
            ...
            gaussK = __gauss(shape=kernelShape, center=center, sx=aGauss['sx'],\
                             sy=aGauss['sy'])
...
def optimalSubtractOnGrid(image, refImage, gaussList = None, bkgDegree = 3,\
    ...
    If gaussList is None (default value), the OIS will try to optimize the value
    of each pixel in the kernel [Bramich, 2010].
    ...
    slc_wborder_y = [slice(max(0, h*i/ny - (kh-1)/2), \
                           min(h, h*(i+1)/ny + (kh-1)/2), None)  for i in range(ny)]
    slc_wborder_x = [slice(max(0, w*i/nx - (kw-1)/2), \
                           min(w, w*(i+1)/nx + (kw-1)/2), None)  for i in range(nx)]
